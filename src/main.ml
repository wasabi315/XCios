...
let parse filename =
  ...
  | Parser.Error ->
    ...
    let msg =
      Printf.sprintf
        "Syntax error in %s : Line %d, Char %d"
        filename
        pos.pos_lnum
...
let gather_filedata entry_file =
  let rec visit file acc =
    ...
    | Some Visited -> acc
  in
  let _, all_data, file_ord_rev = visit entry_file (Idmap.empty, Idmap.empty, []) in
...
let compile path =
  ...
  with
  | ParseError msg | FileError msg | Typing.Error msg ->
    printf "Compile Error : %s" msg;
    print_newline ()
;;

let usage_msg = "[Usage] emfrp-switch [file]..."
let () = Arg.parse [] compile usage_msg
