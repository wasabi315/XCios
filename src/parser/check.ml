...
let check_name_conflict_module elems =
  check_name_conflict module_elem_id elems

let check_name_conflict_state elems =
  check_name_conflict state_elem_id elems

let check_name_conflict_smodule elems =
  check_name_conflict smodule_elem_id elems

let check_name_conflict_file elems =
  check_name_conflict xfrp_elem_id elems
...
let check_nodes out_decls shared_decls nodes newnodes =
  ...
  let get_all_declared_nodes out_decl shared_decl =
    ...
    Idmap.empty
    |> List.fold_right (fun (id, _, _) all_nodes ->
           add id OutputNode all_nodes
         ) out_decl
    |> List.fold_right (fun (id, _, _) all_nodes ->
           add id SharedNode all_nodes
         ) shared_decl
  in
  ...
  let get_all_defined_nodes nodes newnodes =
    ...
    let add_newnode def all_nodes =
      List.fold_left (fun all_nodes (attr, id, _) -> add id attr all_nodes)
        all_nodes def.newnode_binds
    in

    Idmap.empty
    |> Idmap.fold (fun _ def all_nodes ->
           add def.node_id def.node_attr all_nodes
         ) nodes
    |> Idmap.fold (fun _ def all_nodes ->
           add_newnode def all_nodes
         ) newnodes
  in

  let check_header_nodes_defined all_decls all_nodes =
    Idmap.iter (fun id decl_attr ->
        match Idmap.find_opt id all_nodes with
        | Some (def_attr) ->
           if decl_attr != def_attr then
             let msg =
               Format.asprintf "Conflict node attribute : %a" pp_identifier id
             in
             raise (Error msg)
           else ()
        | None ->
           let msg =
             Format.asprintf "Header node is not defined : %a" pp_identifier id
           in
           raise (Error msg)
      ) all_decls
  in
  ...
  let check_undecl_header_nodes all_nodes all_decls =
    Idmap.iter (fun id def_attr ->
        ...
        | _, None ->
           let msg =
             Format.asprintf "Undeclared header node : %a" pp_identifier id
           in
           raise (Error msg)
      ) all_nodes
  in
