...
let calc_typedata all_data file_ord metainfo =
  let visit_tstate typedata file module_name =
    ...
    let () = if num_states = 1 then Hashset.add typedata.singleton_types tstate else () in
    let tag_table, _ =
      idmap_fold_values
        (fun state (tag_table, new_tag) ->
  ...
  let visit_tid typedata file type_name =
    ...
    let is_enum = Idmap.for_all (fun _ tvalue -> tvalue = TUnit) typedef.type_conses in
    let () = if is_enum then Hashset.add typedata.enum_types tid else () in
    let num_conses = Idmap.cardinal typedef.type_conses in
    let () = if num_conses = 1 then Hashset.add typedata.singleton_types tid else () in
    let tag_table, _ =
      Idmap.fold
        (fun id _ (tag_table, new_tag) -> Idmap.add id new_tag tag_table, new_tag + 1)
        typedef.type_conses
        (Idmap.empty, 0)
    in
    ...
    let visit_file nonenum_tid_defs file =
      ...
      List.fold_left
        (fun nonenum_tid_defs type_id ->
          ...
          if is_used && not is_enum
          then (file, typedef) :: nonenum_tid_defs
  ...
  let get_tstate_defs metainfo =
    List.fold_left
      (fun targets (file, module_or_smodule) ->
  ...
  let () =
    Hashtbl.iter
      (fun t _ ->
        match t with
        | TState (file, module_name) -> visit_tstate metainfo.typedata file module_name
        | TId (file, type_name) -> visit_tid metainfo.typedata file type_name
        | TTuple ts -> visit_ttuple metainfo.typedata ts
        | _ -> assert false)
      metainfo.alloc_amount
  in
