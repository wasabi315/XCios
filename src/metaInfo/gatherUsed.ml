...
let update_used all_data expr used =
  let rec rec_f (ast, _) used =
    ...
    | EIf (etest, ethen, eelse) -> used |> rec_f etest |> rec_f ethen |> rec_f eelse
    | ELet (binders, e) ->
      used
      |> List.fold_right (fun binder used -> rec_f binder.binder_body used) binders
      |> rec_f e
    | ECase (e, branchs) ->
      used
      |> rec_f e
      |> List.fold_right (fun branch used -> rec_f branch.branch_body used) branchs
  in
  rec_f expr used
;;
  ...
  | id, ModuleCons (file, _, _, _) ->
    ...
    (match Idmap.find id filedata.xfrp_all with
     | XFRPModule def ->
       ...
       if Idset.mem global_name used then used else visit_module all_data file def used
     | XFRPSModule def ->
       let global_name = conc_id [ file; "smodule"; def.smodule_id ] in
       if Idset.mem global_name used then used else visit_smodule all_data file def used
     | _ -> assert false)
  | _ -> assert false
...
and visit_module_materials all_data consts nodes newnodes used =
  used
  |> Idmap.fold (fun _ def used -> update_used all_data def.const_body used) consts
  |> Idmap.fold (fun _ def used -> visit_node all_data def used) nodes
  |> Idmap.fold (fun _ def used -> visit_newnode all_data def used) newnodes
...
and visit_state all_data def used =
  visit_module_materials all_data def.state_consts def.state_nodes def.state_newnodes used
...
and visit_smodule all_data file def used =
  ...
  |> Idmap.fold
       (fun _ def used -> update_used all_data def.const_body used)
       def.smodule_consts
  |> Idmap.fold (fun _ def used -> visit_state all_data def used) def.smodule_states
;;

let gather all_data entry_file =
  let filedata = Idmap.find entry_file all_data in
  match Idmap.find "Main" filedata.xfrp_all with
  | XFRPModule def -> visit_module all_data entry_file def Idset.empty
  | XFRPSModule def -> visit_smodule all_data entry_file def Idset.empty
  | _ -> assert false
;;
