...
let pp_id_and_type ppf (id, t) = fprintf ppf "%a : %a" pp_identifier id Type.pp_t t

let conc_id idlist =
  let idlist = List.rev idlist in
  match idlist with
  | [] -> ""
  | x :: xs -> List.fold_left (fun acc id -> id ^ "_" ^ acc) x xs
;;
...
let pp_idmap pp_contents ppf idmap =
  let pp_binds ppf (id, x) = fprintf ppf "%a -> %a" pp_identifier id pp_contents x in
...
let idmap_value_list (m : 'a Idmap.t) : 'a list =
  let _, values = Idmap.bindings m |> List.split in
  values
;;
...
type idinfo =
  ...
  | ModeValue of string * string * modev_order * access
...
let pp_idref ppf (id, idinfo) = fprintf ppf "%a(%a)" pp_identifier id pp_idinfo idinfo
...
let rec pp_pattern_ast ppf = function
...
and pp_pattern ppf (ast, t) = fprintf ppf "%a : %a" pp_pattern_ast ast Type.pp_t t
...
let rec pp_expression_ast ppf = function
  | EUniOp (op, e) -> fprintf ppf "@[<2><uniop %a@ %a>@]" pp_uni_op op pp_expression e
  | EBinOp (op, e1, e2) ->
    fprintf
      ppf
      "@[<2><binop %a@ @[%a@ %a@]>@]"
      ...
      pp_expression
      e2
  | EVariant (c, v) -> fprintf ppf "@[<2><variant %a %a>@]" pp_idref c pp_expression v
  ...
  | EAnnot (idref, annot) ->
    fprintf ppf "@[<h><annot %a %@ %a>@]" pp_idref idref pp_annotation annot
  | EFuncall (idref, es) ->
    fprintf
      ppf
      "@[<v 2><funcall %a@;@[<v>%a@]>@]"
...
and pp_branch ppf { branch_pat; branch_body } =
  fprintf ppf "%a -> @[%a@]" pp_pattern branch_pat pp_expression branch_body

and pp_expression ppf (ast, t) = fprintf ppf "%a : %a" pp_expression_ast ast Type.pp_t t
...
let pp_typedef ppf { type_pub; type_id; type_conses } =
  ...
  fprintf ppf "constructors: @[%a@]@]@;" (pp_idmap Type.pp_t) type_conses;
  fprintf ppf "}@]"
;;
...
let pp_mode_annot ppf = function
...
let pp_fundef ppf { fun_pub; fun_id; fun_rettype; fun_params; fun_body } =
  ...
  fprintf ppf "params: @[%a@]@;" (pp_list_comma pp_id_and_type) fun_params;
  fprintf ppf "type: @[%a@]" Type.pp_t fun_rettype;
...
let pp_node_decl ppf (id, init, t) =
  ...
  fprintf
    ...
    pp_identifier
    id
    (pp_opt pp_expression pp_init_none)
    init
    Type.pp_t
    t
;;
...
(* newnode *)
type newnode_bind =
  | NBPass of identifier
  | NBDef of identifier

let pp_newnode_bind ppf = function
  | NBPass id -> fprintf ppf "&%a" pp_identifier id
  | NBDef id -> fprintf ppf "%a" pp_identifier id
;;

type newnode =
  { newnode_id : identifier (* generated by compiler *)
  ; newnode_binds : (nattr * newnode_bind * Type.t) list
...
let pp_newnode ppf def =
  let pp_newnode_bind ppf (attr, id, t) =
    fprintf ppf "%a(%a): %a" pp_newnode_bind id pp_nattr attr Type.pp_t t
  in
  fprintf ppf "@[<v>newnode: {@;<0 2>";
  fprintf ppf "@[<v>bind: @[%a@]@;" (pp_list_comma pp_newnode_bind) def.newnode_binds;
...
type xfrp_module =
  ...
  ; module_mode_annots : mode_annot Idmap.t
...
let pp_xfrp_module ppf def =
  ...
  fprintf ppf "mode_annot: @[%a@]@;" (pp_idmap pp_mode_annot) def.module_mode_annots;
  ...
  fprintf ppf "consts_ord: @[%a@]@;" (pp_list_comma pp_identifier) def.module_consts_ord;
  fprintf ppf "update_ord: @[%a@]@]@;" (pp_list_comma pp_identifier) def.module_update_ord;
  fprintf ppf "}@]"
;;
...
type state =
  ...
  ; state_mode_annots : mode_annot Idmap.t
...
let pp_state ppf def =
  ...
  fprintf ppf "mode_annot: @[%a@]@;" (pp_idmap pp_mode_annot) def.state_mode_annots;
  ...
  fprintf ppf "consts_ord: @[%a@]@;" (pp_list_comma pp_identifier) def.state_consts_ord;
  fprintf ppf "update_ord: @[%a@]@]@;" (pp_list_comma pp_identifier) def.state_update_ord;
  fprintf ppf "}@]"
;;
...
let pp_xfrp_smodule ppf def =
  ...
  fprintf ppf "params: @[%a@]@;" (pp_list_comma pp_id_and_type) def.smodule_params;
  fprintf ppf "in: @[%a@]@;" (pp_list_comma pp_node_decl) def.smodule_in;
  fprintf ppf "out: @[%a@]@;" (pp_list_comma pp_node_decl) def.smodule_out;
  fprintf ppf "consts: @[%a@]@;" (pp_idmap pp_constdef) def.smodule_consts;
  fprintf ppf "states: @[%a@]@;" (pp_idmap pp_state) def.smodule_states;
  fprintf
    ppf
    "consts_ord: @[%a@]@]@;"
...
let pp_xfrp ppf def =
  ...
  fprintf ppf "smodules: @[%a@]@]@;" (pp_idmap pp_xfrp_smodule) def.xfrp_smodules;
  fprintf ppf "}@]"
;;
