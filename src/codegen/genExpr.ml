...
let get_pattern_conds_binds metainfo gen_target pattern =
  ...
  let rec rec_f gen_target (ast, tpat) (conds, binds) =
    ...
    let f_const literal =
      let gen_cond ppf () =
        fprintf ppf "%a == %a"
          gen_target () gen_literal literal
      in
      (gen_cond :: conds, binds)
    in

    let f_tuple subpats =
      let patterns_with_position =
        List.mapi (fun pos pattern -> (pattern, pos)) subpats
      in
      List.fold_left (fun conds_binds (pattern, pos) ->
          let gen_subtarget ppf () =
            fprintf ppf "%a->member%a"
              gen_target () pp_print_int pos
          in
    ...
    let f_valuecons cons_id subpat =
      ...
      | TBool | TInt | TFloat | TId _ | TTuple _ ->
         let gen_subtarget ppf () =
           fprintf ppf "%a->value.%a"
             gen_target () pp_print_string cons_id
         in
    ...
    let f_statecons cons_id subpat =
      ...
      in
      let param_ids =
        Hashtbl.find tstate_param_ids tpat |> Idmap.find cons_id
      in
      match param_ids with
      | [] -> (conds, binds)
      | [param_id] ->
         let gen_subtarget ppf () =
           fprintf ppf "%a->params.%a.%a"
             gen_target ()
             pp_print_string cons_id
             pp_print_string param_id
         in
         rec_f gen_subtarget subpat (conds, binds)
      | param_ids ->
         List.fold_left (fun (conds, binds) param_id ->
             let gen_subtarget ppf () =
               fprintf ppf "%a->params.%a.%a"
                 gen_target ()
                 pp_print_string cons_id
    ...
    | PVariant (idref, subpat) ->
       ...
       match idinfo with
       | ValueCons _ ->
          f_valuecons cons_id subpat
       | StateCons _ ->
          f_statecons cons_id subpat
       | _ -> assert false
  in
...
let get_expr_generator metainfo codegen_ctx expr : writer list * writer =
  ...
  let rec rec_f (ast, tast) body_writers  =
    ...
    let f_variant idref value =
      ...
      let f_statecons file module_id tvalue =
        let (body_writers, gen_args) =
          match tvalue with
          | TUnit | TBool | TInt | TFloat | TId _ ->
             (body_writers, gen_value)
          | TTuple ts ->
             let varname = make_tmpvar_name () in
             let gen_prepare_args ppf () =
               fprintf ppf "%a %a = %a;"
                 (gen_value_type metainfo) tvalue
                 pp_print_string varname
                 gen_value ()
             in
             let tuple_size = List.length ts in
             let tuple_members =
               List.init tuple_size (fun i -> asprintf "member%d" i)
             in
             let gen_args ppf () =
               pp_print_list pp_print_string ppf tuple_members
    ...
    let f_const literal =
      (body_writers, (fun ppf () -> gen_literal ppf literal))
    in
    ...
    let f_retain () =
      ...
      let gen_expr ppf () =
        ...
        let f_state_node state_id nattr node_id =
          match nattr with
          | OutputNode | SharedNode ->
             fprintf ppf "memory->%s[!current_side]" node_id
          | NormalNode ->
             fprintf ppf "memory->statebody.%s.%s[!current_side]"
               state_id node_id
          | _ -> assert false
        in
    ...
    let f_if etest ethen eelse =
      ...
      let gen_body ppf () =
        fprintf ppf "@[<v>";
        fprintf ppf "%a %a;"
          (gen_value_type metainfo) tast pp_print_string ret_varname;
        if testbody_writers = [] then () else
          fprintf ppf "@,%a" (exec_all_writers ()) testbody_writers;
        fprintf ppf "@,@[<v>";
    ...
    let f_case target branchs =
      ...
      let gen_body_head ppf () =
        fprintf ppf "@[<v>";
        if tbody_writers = [] then () else
          fprintf ppf "%a@," (exec_all_writers ()) tbody_writers;
        fprintf ppf "%a %a = %a;"
          (gen_value_type metainfo) ttarget
          pp_print_string target_varname
          gen_texpr ();
        fprintf ppf "@]"
      in

      let gen_target ppf () =
        pp_print_string ppf target_varname
      in
      ...
      let f_single_branch branch =
        ...
        let gen_body ppf () =
          ...
          if binds = [] then () else
            fprintf ppf "@,%a" (exec_all_writers ()) binds;
          if pbody_writers = [] then () else
            fprintf ppf "@,%a" (exec_all_writers ()) pbody_writers;
          fprintf ppf "@]"
        in
      ...
      let f_multiple_branch branchs =
        ...
        let gen_branch_body ppf (binds, pbody_writers, gen_pexpr) =
          ...
          if pbody_writers = [] then () else
            fprintf ppf "%a@," (exec_all_writers ()) pbody_writers;
          fprintf ppf "%a = %a;"
            pp_print_string ret_varname gen_pexpr ();
          fprintf ppf "@]"
        in
        ...
        let gen_body ppf () =
          ...
          List.iteri (fun pos branch ->
              ...
              in
              if pos = 0 then
                fprintf ppf "if (@[<hov>%a@]) {@;<0 2>" gen_condexpr conds
              else if pos = branch_size - 1 then
                fprintf ppf " else {@;<0 2>"
              else
                fprintf ppf " else if (@[<hov>%a@]) {@;<0 2>" gen_condexpr conds;
              fprintf ppf "@[<v>%a@]@,"
                gen_branch_body (binds, pbody_writers, gen_pret);
              fprintf ppf "}"
            ) branchs;
          fprintf ppf "@]"
        in
