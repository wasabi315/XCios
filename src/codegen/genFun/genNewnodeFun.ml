...
let define_newnode_fun metainfo generator fun_writers =
  ...
  let params =
    List.map2 (fun (id, t) expr -> id, t, expr) param_sig newnode.newnode_margs
  in
  let inputs =
    List.map2 (fun (id, t) expr -> id, t, expr) in_sig newnode.newnode_inputs
  in
  let outputs =
    List.map2
      (fun (from_id, _) (attr, to_id, t) -> from_id, attr, to_id, t)
      out_sig
  ...
  let define_param_fun (param_id, _, expr) fun_writers =
    let gen_funname ppf () =
      fprintf ppf "static void param_%a_%a" gen_instance_name () pp_print_string param_id
    in
    let generator =
    ...
    let gen_definition ppf () =
      ...
      let gen_body_param ppf params =
        let gen_single ppf (param_id, t, _) =
          ...
          let gen_address ppf () = (gen_param_address param_id) ppf () in
      ...
      let gen_body ppf () =
        fprintf ppf "@[<v>";
        if params = [] then () else fprintf ppf "%a@," gen_body_param params;
        if inputs = [] then () else fprintf ppf "%a@," gen_body_input inputs;
        fprintf ppf "if (%a) {@;<0 2>" gen_init ();
        ...
        fprintf
          ...
          gen_instance_address
          ();
        if outputs = [] then () else fprintf ppf "@,%a" gen_body_output outputs;
        fprintf ppf "@]"
      in
