...
let gen_consdef_body gen_core typename ppf () =
  ...
  let gen_alloc_loop_body ppf () =
    ...
    fprintf
      ...
      (gen_codeblock
         ...
         (fun ppf () -> fprintf ppf "@[<h>x = %s + %s; break;@]" memory_var counter_var))
      ();
    fprintf ppf "@]"
  in
  fprintf ppf "@[<v>";
  ...
  fprintf
    ppf
    "@,%a"
    (gen_codeblock (fun ppf () -> fprintf ppf "@[<h>while (1)@]") gen_alloc_loop_body)
    ();
  ...
  let define_single_cons cons_id vtype fun_writers =
    let gen_funname ppf () =
      fprintf
        ...
        tid
        gen_tid_consname
        (file, type_id, cons_id)
    in
    let gen_prototype ppf () =
      let gen_prototype_params ppf () =
        if vtype = TUnit then () else (gen_value_type metainfo) ppf vtype
      in
      fprintf ppf "@[<h>%a(%a);@]" gen_funname () gen_prototype_params ()
    in
    let gen_definition_head ppf () =
      let gen_definition_params ppf () =
        if vtype = TUnit
        then ()
        else fprintf ppf "%a value" (gen_value_type metainfo) vtype
      in
      fprintf ppf "@[<h>%a(%a)@]" gen_funname () gen_definition_params ()
    in
    let gen_definition_body_core ppf () =
      ...
      let writers =
        ...
        then writers
          ...
          let writer ppf () = fprintf ppf "x->value.%s = value;" cons_id in
          writer :: writers)
      in
...
let define_ttuple_cons metainfo types fun_writers =
  ...
  let gen_definition_params ppf () =
    pp_print_list
      (fun ppf (t, pos) ->
        fprintf ppf "%a member%a" (gen_value_type metainfo) t pp_print_int pos)
      ppf
      types_with_position
  ...
  let gen_definition_body_core ppf () =
    fprintf ppf "@[<v>";
    pp_print_list
      (fun ppf (_, pos) -> fprintf ppf "x->member%d = member%d;" pos pos)
      ppf
      types_with_position;
    fprintf ppf "@]"
  in
...
let define_tstate_conses metainfo (file, xfrp_smodule) fun_writers =
  ...
  let define_single_cons state fun_writers =
    ...
    let gen_prototype_params ppf () =
      pp_print_list
        (fun ppf (_, t) -> (gen_value_type metainfo) ppf t)
        ppf
        state_params
        ~pp_sep:pp_print_commaspace
    in
...
let define_gcfun typename gen_recmark_opt gen_recfree_opt fun_writers =
  let gen_funname_mark ppf () = fprintf ppf "static void mark_%s" typename in
  let gen_markfun_prototype ppf () =
    fprintf ppf "%a(struct %a*, int);" gen_funname_mark () pp_print_string typename
  in
  let gen_markfun_definition ppf () =
    let gen_markfun_definition_body ppf () =
      fprintf ppf "@[<v>";
      fprintf ppf "if (mark > x->mark) { x->mark = mark; }";
      (pp_opt (fun ppf writer -> fprintf ppf "@,%a" writer ()) pp_none)
        ppf
        gen_recmark_opt;
      fprintf ppf "@]"
    in
    (gen_codeblock
       (fun ppf () ->
         fprintf
           ppf
           "@[<h>%a(struct %a* x, int mark)@]"
           gen_funname_mark
           ()
           pp_print_string
           typename)
       gen_markfun_definition_body)
      ppf
      ()
  in
  let gen_funname_free ppf () = fprintf ppf "static void free_%s" typename in
  let gen_freefun_prototype ppf () =
    fprintf ppf "%a(struct %a*);" gen_funname_free () pp_print_string typename
  in
...
let define_tid_gcfun metainfo (file, typedef) fun_writers =
  ...
  let reccall_branchs =
    Idmap.fold
      (fun cons_id vtype branchs ->
        ...
        | TId (file, type_id) ->
          if Hashset.mem enum_types vtype
          then branchs
          else (
  ...
  let gen_recmark_opt, gen_recfree_opt =
    if reccall_branchs = []
    then None, None
    else if Hashset.mem singleton_types tid
    then (
      let cons_id, vtypename =
        ...
        | _ -> assert false
      in
      let gen_recmark ppf () = gen_funcall_mark ppf (cons_id, vtypename) in
      let gen_recfree ppf () = gen_funcall_free ppf (cons_id, vtypename) in
      ...
      let gen_reccall gen_funcall ppf () =
        ...
        fprintf ppf "@]"
      in
      let gen_recmark ppf () = (gen_reccall gen_funcall_mark) ppf () in
      let gen_recfree ppf () = (gen_reccall gen_funcall_free) ppf () in
      Some gen_recmark, Some gen_recfree)
  in
...
let define_ttuple_gcfun metainfo types fun_writers =
  ...
  let reccalls =
    List.fold_right
      (fun (memtype, pos) reccalls ->
        match memtype with
        | TBool | TInt | TFloat | TUnit -> reccalls
        | TId (file, type_id) ->
          if Hashset.mem enum_types memtype
          then reccalls
          else (
            let memtypename = asprintf "%a" gen_tid_typename (file, type_id) in
            (memtypename, pos) :: reccalls)
        | TTuple types ->
          let memtypename = asprintf "%a" gen_ttuple_typename types in
          (memtypename, pos) :: reccalls
        | _ -> assert false)
      types_with_position
      []
  ...
  let gen_recmark_opt, gen_recfree_opt =
    ...
    then None, None
      ...
      let gen_recmark ppf () = pp_print_list gen_funcall_mark ppf reccalls in
      let gen_recfree ppf () = pp_print_list gen_funcall_free ppf reccalls in
      Some gen_recmark, Some gen_recfree)
  in
  define_gcfun typename gen_recmark_opt gen_recfree_opt fun_writers
;;
...
let define_tstate_gcfun metainfo (file, xfrp_smodule) fun_writers =
  ...
  let get_state_reccalls state =
    let state_id = state.state_id in
    List.fold_right
      (fun (pid, ptype) reccalls ->
        match ptype with
        | TBool | TInt | TFloat | TUnit -> reccalls
        | TId (file, type_id) ->
          if Hashset.mem enum_types ptype
          then reccalls
          else (
            let ptypename = asprintf "%a" gen_tid_typename (file, type_id) in
            (state_id, pid, ptypename) :: reccalls)
        | TTuple types ->
          let ptypename = asprintf "%a" gen_ttuple_typename types in
          (state_id, pid, ptypename) :: reccalls
        | _ -> assert false)
      state.state_params
      []
  ...
  let gen_funcall_mark ppf (state_id, pid, ptypename) =
    fprintf ppf "mark_%s(x->params->%s->%s, mark);" ptypename state_id pid
  in
  let gen_funcall_free ppf (state_id, pid, ptypename) =
    fprintf ppf "free_%s(x->params->%s->%s, mark);" ptypename state_id pid
  in
  let gen_recmark_opt, gen_recfree_opt =
    if reccall_branchs = []
    then None, None
    else if Hashset.mem singleton_types tstate
    then (
      let reccalls =
        match reccall_branchs with
        | [ (_, reccalls) ] -> reccalls
