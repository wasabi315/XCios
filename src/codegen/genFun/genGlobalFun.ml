...
let define_refresh_mark_fun metainfo fun_writers =
  ...
  let gen_funname ppf () =
    fprintf ppf "static void refresh_mark"
  in
  ...
  let gen_definition ppf () =
    ...
    let gen_body_loop_tstate ppf (file, xfrp_smodule) =
      let typename =
        asprintf "%a" gen_tstate_typename (file, xfrp_smodule.smodule_id)
      in
      gen_body_loop ppf typename
    in
    ...
    let gen_body ppf () =
      ...
      if nonenum_tid_defs = [] then () else
        fprintf ppf "@,%a"
          (pp_print_list gen_body_loop_tid) nonenum_tid_defs;
      if tuple_types = [] then () else
        fprintf ppf "@,%a"
          (pp_print_list gen_body_loop_ttuple) tuple_types;
      if tstate_defs = [] then () else
        fprintf ppf "@,%a"
          (pp_print_list gen_body_loop_tstate) tstate_defs;
      fprintf ppf "@]"
    in
...
let add_extern_prototype metainfo prototype_writers =
  ...
  let gen_protype_args ppf signature =
    (pp_print_list (fun ppf (_, t) ->
         fprintf ppf "%a*" (gen_value_type metainfo) t
  ...
  prototype_writers
  |> List.cons gen_input_prototype
  |> List.cons gen_output_prototype
...
let gen_activate_fun ppf metainfo =
  ...
  let gen_head ppf () =
    fprintf ppf "void activate(@[<h>%a@])" gen_head_arg ()
  in

  let gen_body_const_init ppf (file, const) =
    let global_name =
      asprintf "%a" gen_global_constname (file, const.const_id)
    in
    let gen_body ppf () =
      fprintf ppf "init_%s();" global_name
    in
    let gen_address ppf () =
      pp_print_string ppf global_name
    in
    let gen_life ppf () =
  ...
  let const_remark_writers =
    List.fold_left (fun writers (file, const) ->
        let gen_address ppf () =
          gen_global_constname ppf (file, const.const_id)
        in
        let gen_life ppf () =
          fprintf ppf "clock + period"
        in
  ...
  let gen_body_loop ppf () =
    fprintf ppf "clock = 0;";
    if const_remark_writers = [] then () else
      fprintf ppf "@,%a" (exec_all_writers ()) const_remark_writers;
    fprintf ppf "@,clock = 1;";
  ...
  let gen_body ppf () =
    ...
    fprintf ppf "@,clock = 0;";
    if param_sig = [] then () else
      begin
        let gen_sig_init ppf (id, _) =
          fprintf ppf "memory->%s = %s;" id id
        in
