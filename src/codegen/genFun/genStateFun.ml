...
let define_state_fun metainfo file module_id state fun_writers =
  ...
  let global_statename = asprintf "%a" gen_global_statename (file, module_id, state_id) in
  let gen_memorytype ppf () = gen_module_memory_type ppf (file, module_id) in
  let define_state_const_fun const fun_writers =
    let gen_funname ppf () =
      fprintf ppf "static void init_%s_%s" global_statename const.const_id
    in
    let gen_address ppf () =
  ...
  let define_state_newnode_fun newnode fun_writers =
    ...
    in
    let gen_instance_name ppf () = fprintf ppf "%s_%s" global_statename instance_id in
    let generator =
  ...
  let define_state_switch_fun expr fun_writers =
    let gen_funname ppf () = fprintf ppf "static void update_%s_state" global_statename in
